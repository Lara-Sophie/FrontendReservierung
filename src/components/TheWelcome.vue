    ...
    <div class="welcome-container">
        ...
        <div class="content">
            ...
            <div>
                ...
                <button type="button" data-user-id="123" @click="navigateToAboutPage">reservieren</button>
                <p>Wenn du noch nicht angemeldet bist dann registriere dich bitte hier:</p>
                <button type="button" @click="navigateToRegister">Registrieren</button>
            </div>
        </div>
    </div>
</template>
...
const navigateToAboutPage = (event: MouseEvent) => {
    // Extrahiere den Wert des "data-user-id" Attributs
    const userId = (event.target as HTMLElement).getAttribute('data-user-id');

    // Überprüfe, ob userId vorhanden ist, bevor du weitermachst
    if (userId) {
        const router = useRouter();
